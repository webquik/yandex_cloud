...
def not_found(e): 
  return render_template('error.html', err_text="Incorrect call. Status equal 404")
...
def get_weather():
    try:
        if request.method == 'POST':
            ...
            if city_id.empty == False: # & isinstance(city_id, pd.Series):  
                try:
                    city_id = city_id.values[0]
        ...
        return render_template('error.html', err_text="Some bugs on server")
